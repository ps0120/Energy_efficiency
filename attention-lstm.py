...
def save_info():
    ...
    try:
        ...
        exact_date_mask = (filtered_by_type['Year'] == query_year) & \
                          (filtered_by_type['Month'] == month_info) & \
                          (filtered_by_type['Day'] == day_info)
            ...
            if len(month_data) > 0:
                ...
                if len(year_data) > 0:
                    ...
                    if len(filtered_by_type) > 0:
                        ...
                        print("\n--- No Matching Data Found in Entire History ---\n")
                        energy = 0 
...
def train():
    ...
    # Convert 3D output back to 2D using Flatten (safest for cross-Keras compatibility)
    ...
    score = model.evaluate(test_X, test_y, batch_size=32, verbose=1)  
    # Make a prediction by using the training model
    ...
    print('Test Accuracy (100%% - MAPE): %.2f%%' % accuracy)
    
    r_squared = r2_score(inv_y, inv_yhat)
    print('R2 Score (Goodness of Fit): %.4f' % r_squared)
    ...
    plt.plot(dataset["TimeStamp"][start_index:start_index+len(inv_y)], arr_forecast[:len(inv_y)], 'r-', label="Prediction", linewidth=2)
