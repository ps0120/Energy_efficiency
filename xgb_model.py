...
def train():
    ...
    # XGBoost doesn't need 3D input like LSTM, so we keep 2D shape
    # train_X shape: (samples, features)
    # No need to reshape for XGBoost
    ...
    print(f'XGBoost Test Score (R²): {score:.6f}')
    
    # Make a prediction by using the XGBoost model
...
screen.title("MMU Energy Prediction (XGBoost)")
...
tv2.configure(xscrollcommand=treescrollx2.set,yscrollcommand=treescolly2.set)
